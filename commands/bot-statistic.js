...
**`)
              .setColor(Colors.Green)
              .setTimestamp()
            await interaction.editReply({ content: "**✔️ Data Günellendi.**", embeds: [embed2] }).catch(err => { })
      ...
      col.on('end', async (button) => {
        link_button = new ActionRowBuilder().addComponents(
          new ButtonBuilder()
            ...
            .setCustomId("Refresh")
            .setDisabled(true))
        return interaction.editReply({ content: "**Süreniz Sona Erdi!**", components: [link_button] }).catch(err => { })
      })
    }).catch(err => { })
  },
};
